...
public class ServerConnectionHandler implements HttpHandler {
    ...
    private static void registerUser() throws IOException {
        final JsonObject requestJsonObject = requestBodyJson.getAsJsonObject();
        boolean isValidRequest = membersMatch(requestJsonObject.keySet(),
        ...
        if (isValidRequest) {
            ...
            isValidRequest = (isValidRequest && phoneNumberStr.length() == 10 );
            // this could probably be written better, sorry... -Kyle
            if (isValidRequest) {
                int result = 0;
    ...
    private static void verifyRowExists() throws IOException {
        final JsonObject requestJsonObject = requestBodyJson.getAsJsonObject();
        final boolean isValidRequest = membersMatch(requestJsonObject.keySet(), "query");
        if (isValidRequest) {
            String results = "";
            final String queryString = requestJsonObject.get("query").getAsString();
            try (DatabaseConnection db = new DatabaseConnection()) {
